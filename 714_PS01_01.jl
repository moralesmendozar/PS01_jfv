...
function define_params(w)
	...
	K_prime_grid=zeros(Float64,Nk_prime)
	l=1
	for iK=1:Nk-1
		K_prime_grid[l:l+new_points+1]=range(K_grid[iK],stop=K_grid[iK+1],length=new_points+2)
		l=l+new_points+1
	end
...
function VFI(V_init::Array{Float64},p::parameters_type)
	...
	#Optimal policies
	K_prime_arr=Array{Float64}(undef, p.Nk,p.Nz)#Array(Float64,p.Nk,p.Nz)
	iK_prime_arr=Array{Int64}(undef, p.Nk,p.Nz)#Array(Int64,p.Nk,p.Nz)
	Div_arr=Array{Float64}(undef, p.Nk,p.Nz)#Array(Float64,p.Nk,p.Nz)
		...
		if acceleration==0 #regular step of VFI
			...
			for iK=1:p.Nk
				...
				for iz=1:p.Nz
					...
					#println("size of EV_interp[:,iz]: ",size(EV_interp[:,iz]))
					#println("EV_interp[:,iz]: ", EV_interp[:,iz])
					#V_cur=div.+p.M.*max(EV_interp[:,iz],0)
					vectemp = [max(EV_interp[c,iz],0) for c in 1:size(EV_interp,1)]
					#println("vectemp[1:5] = ",vectemp[1:5])
					V_cur=div.+p.M.*vectemp

					#pos=indmax(V_cur)
					#println("size of V_cur: ",size(V_cur))
					#println("V_cur[1:5] = ",V_cur[1:5])
					#println("findmax(V_cur') = ",findmax(V_cur'))
					pos=findmax(V_cur')[2][2]
...
@debug function dstrb_no_entry(iK_prime::Array{Int64},p::parameters_type)
