...
function multi_grid(economy, steadyState, nMidPoints::Array)
    ...
    capitalMinimum = 0.7 * capitalSteadyState
    capitalMaximum = 1.3 * capitalSteadyState
    ...
    for iGrid in 1:length(nMidPoints)
        ...
        # 3.1. Grid Capital
        # ----------------
        global vGridCapital = exercise7aux.gridmake(capitalMinimum, capitalMaximum, nMidPoints[iGrid])
        nGridCapital = length(vGridCapital)

        # -----------------
        # 3.2. Pre-allocation
        # -----------------
        global tValueFunction = fill(utilitySteadyState, nGridCapital, nGridZ, nGridA)
        tValueFunctionNew = zeros(nGridCapital, nGridZ, nGridA)
        global tPolicyFunctionIndex = zeros(Int64,nGridCapital,nGridZ, nGridA)
