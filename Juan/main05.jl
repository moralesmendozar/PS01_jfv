...
@time tValueFunction, tCapitalPolicy, vGridCapital, vMaxDifference = exercise3.fixed_grid(villaverdeEconomy, villaverdeSteadyState, nGridCapital)
...
#=
# 3.3.
pLaborOnePolicyFixedGrid = plot(vGridCapital, tLaborOnePolicy[:,1,1], label = "z_1, A_1", xlabel = "Capital", legend=:bottomright)
plot!(vGridCapital, tLaborOnePolicy[:,end,1],label = "z_5, A_1")
plot!(vGridCapital, tLaborOnePolicy[:,1,end], label = "z_1, A_5")
plot!(vGridCapital, tLaborOnePolicy[:,end,end], label = "z_5, A_3")

# 3.4.
pLaborTwoPolicyFixedGrid = plot(vGridCapital, tLaborTwoPolicy[:,1,1], label = "z_1, A_1", xlabel = "Capital", legend=:bottomright)
plot!(vGridCapital, tLaborPolicy[:,end,1],label = "z_5, A_1")
plot!(vGridCapital, tLaborOnePolicy[:,1,end], label = "z_1, A_5")
plot!(vGridCapital, tLaborOnePolicy[:,end,end], label = "z_5, A_3")
=#
# 3.5.
...
# 6. Multigrid
# --------------
multigrid_scheme = @with_kw (
                        # (a) length of each grid
                        nGridCapitalFirst = 100,
                        nGridCapitalSecond = 500,
                        nGridCapitalLast = 50000,

                         # (b) switching grid at iteration ...
                         cutoffFirstGrid = 100,
                         cutoffSecondGrid = 200
)

mutigridScheme = multigrid_scheme()

@time tValueFunction, tCapitalPolicy, vGridCapital, vMaxDifference = exercise7.multigrid(villaverdeEconomy, villaverdeSteadyState, mutigridScheme)
