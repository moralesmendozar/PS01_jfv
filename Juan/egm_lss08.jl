...
function egm_labor_ss(economy, steadyState, vGridCapitalNext::Array, vInitialGuess::Array)
    ...
    tMarketResourcesNext = repeat(mMarketResourcesNext, 1,1, nGridA)
    ...
    tValueEndogenousNew = zeros(nGridCapital, nGridZ, nGridA)
    ...
    while(maxDifference > tolerance)
        ...
        for iA in 1:nGridA
            ...
            for iZ in 1:nGridZ
                ...
                itp = LinearInterpolation(tMarketResourcesEndogenous[:,iZ, iA], tValueEndogenous[:, iZ, iA]; extrapolation_bc = Line() )

                vAux = itp(tMarketResourcesNext[:,iZ,iA])

                tValueEndogenousNew[:,iZ,iA] = vAux
        ...
        tValueFunctionTilde = (1-β)*tValueEndogenousNew

    return tValueFunctionTilde, tMarketResourcesEndogenous
